...
object First {
  def main (args: Array[String]){
    ...
    val conf = new SparkConf().setMaster("spark://"+ip+":7077").setAppName("TestMaven")
    val sc = new SparkContext(conf)

//    val rdd = sc.textFile("hdfs://"+ip+"/mp/hello.txt")
    val rdd = sc.textFile("hdfs://"+ip+"/test")
